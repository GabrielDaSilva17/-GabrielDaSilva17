...
// --- INICIALIZAÇÃO ÚNICA E UNIFICADA DO SISTEMA ---
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (savedLang) {
        // Se houver idioma salvo, atualiza o estado global
        ...
        // Atualiza a tradução e carrega a página inicial
        updateUIStrings();
        loadPage("home");
    } else {
        // Se NÃO existir cookie de idioma, pré-carrega o português como padrão
        currentLang = "pt";
        updateUIStrings();
        loadPage("home");

        // Remove a classe hidden do popup para que o usuário escolha o idioma
        if (popup) {
            popup.classList.remove("lang-popup-hidden");
        }
    }

    // 2. Configura a ação de clique nos botões do POPUP de idiomas
    document.querySelectorAll(".popup-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            const selected = btn.getAttribute("data-lang-choice");

            // Define o cookie de idioma para expirar em 1 dia
            setLanguageCookie(selected, 1);

            // Atualiza o estado global
            currentLang = selected;

            // Sincroniza visualmente os botões do cabeçalho do site
